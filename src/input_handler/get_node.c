...
/*   Updated: 2023/12/06 18:24:58 by johnavar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_scmd	*get_node(t_data *mish, t_scmd *node, char **cmds[2], int *i)
{
	if (cmds[1][*i] && !(*i == 1 && cmds[1][*i - 1][0] == '|'))
	{
		if (cmds[1][*i][0] == '>' && cmds[1][*i + 1]
			...
			node = redir_out(mish, node, cmds[0], i);
		else if (cmds[1][*i][0] == '<' && cmds[1][*i + 1]
			&& cmds[1][*i + 1][0] == '<')
			node = redir_in_heredoc(node, cmds[0], i);
		else if (cmds[1][*i][0] == '<')
			node = redir_in(mish, node, cmds[0], i);
		else if (cmds[1][*i][0] != '|')
